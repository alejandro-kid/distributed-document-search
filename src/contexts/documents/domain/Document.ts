...
export class Document extends AggregateRoot {
  ...
  constructor(id: DocumentId, title: string, content: string, createdAt: Date) {
    super();
    this.id = id;
    this.title = title;
  ...
  static create(props: { title: string; content: string }): Document {
    ...
    const document = new Document(DocumentId.generate(), props.title, props.content, new Date());

    // Emit domain event
    document.record(new DocumentIndexedEvent(document.id.value, document.title, document.content, document.createdAt));

    return document;
  }

  static fromPrimitives(plainData: { id: string; title: string; content: string; createdAt: Date }): Document {
    return new Document(new DocumentId(plainData.id), plainData.title, plainData.content, plainData.createdAt);
  }
